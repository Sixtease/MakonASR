...
lmplz --order 3 --memory 90% --temp_prefix "$wd" < "$c0" > "$wd/lm0".arpa
lmplz --order 3 --memory 90% --temp_prefix "$wd" < "$c1" > "$wd/lm1".arpa
#lmplz --order 3 --memory 90% --temp_prefix "$wd" < "$c2" > "$wd/lm2".arpa
lmplz --order 3 --memory 90% --temp_prefix "$wd" < "$c3" > "$wd/lm3".arpa
#vyslov.pl < "$wd/merged-wordlist" > "$DEST_WL_DIR/wl-test-phonet"

lmplz --order 3 --memory 90% --temp_prefix "$wd" < "$c0"b > "$wd/lm0"b.arpa
lmplz --order 3 --memory 90% --temp_prefix "$wd" < "$c1"b > "$wd/lm1"b.arpa
#lmplz --order 3 --memory 90% --temp_prefix "$wd" < "$c2"b > "$wd/lm2"b.arpa
lmplz --order 3 --memory 90% --temp_prefix "$wd" < "$c3"b > "$wd/lm3"b.arpa
...
ngram -order 3 -lm "$wd/lm0".arpa  -lambda 0.0001 -mix-lm "$wd/lm1".arpa  -mix-lm2 "$wd/lm3".arpa  -mix-lambda2 0.7999 -write-lm "$DEST_LM_DIR/tg.arpa"
ngram -order 3 -lm "$wd/lm0"b.arpa -lambda 0.0001 -mix-lm "$wd/lm1"b.arpa -mix-lm2 "$wd/lm3"b.arpa -mix-lambda2 0.7999 -write-lm "$DEST_LM_DIR/tgb.arpa"
